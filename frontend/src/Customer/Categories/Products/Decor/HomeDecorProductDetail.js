...
const HomeDecorProductDetail = ({ product, onAddToCart, onToggleWishlist, cartItems, wishlistItems, navigateTo, onViewProduct }) => {
    ...
    useEffect(() => {
        const fetchRelatedProducts = async () => {
            if (!product) return;
            try {
                const response = await fetch('http://localhost:5000/api/products/category/Home%20Decor');
                if (!response.ok) {
    ...
    if (!product) {
        return (
            <div className="container">
                ...
                <a href="#homedecor" onClick={(e) => { e.preventDefault(); navigateTo('homedecor'); }} className="back-link">&larr; Back to Home Decor</a>
            </div>
        );
    }
    ...
    return (
        <div className="container">
            <a 
                href="#homedecor" 
                onClick={(e) => { e.preventDefault(); navigateTo('homedecor'); }} 
                className="back-link"
            >
